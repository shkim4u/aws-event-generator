...
import com.aws.proserve.korea.network.syslog.utils.IoUtils;

/**
 * See <a href="http://tools.ietf.org/html/rfc6587">RFC 6587 - Transmission of
 * Syslog Messages over TCP</a>
 *
...
public class TcpSyslogMessageSender extends AbstractSyslogMessageSender {
	...
	// use the CR LF non transparent framing as described in "3.4.2.
		...
		try {
			...
			for (int i = 0; i <= maxRetryCount; i++) {
				try {
					...
					// [2018-10-22] Kim, Sang Hyoun: To calculate sent bytes.
					ByteArrayOutputStream baos = new ByteArrayOutputStream();
					Writer bout = new OutputStreamWriter(baos, UTF_8);
					message.toSyslogMessage(messageFormat, bout);
//					message.toSyslogMessage(messageFormat, writer);
					byte[] bytes = baos.toByteArray();
					writer.write(new String(bytes, UTF_8));
	...
	private synchronized void ensureSyslogServerConnection() throws IOException {
		...
		if (socket != null && !Objects.equals(socket.getInetAddress(), inetAddress)) {
			logger.info("InetAddress of the Syslog Server have changed, create a new connection. " + "Before="
				+ socket.getInetAddress() + ", new=" + inetAddress);
			IoUtils.closeQuietly(socket, writer);
			writer = null;
			socket = null;
		}
	...
	public String toString() {
		return getClass().getName() + "{" + "syslogServerHostname='" + this.getSyslogServerHostname() + '\''
			+ ", syslogServerPort='" + this.getSyslogServerPort() + '\'' + ", ssl=" + ssl + ", maxRetryCount="
			+ maxRetryCount + ", socketConnectTimeoutInMillis=" + socketConnectTimeoutInMillis + ", defaultAppName='"
			+ defaultAppName + '\'' + ", defaultFacility=" + defaultFacility + ", defaultMessageHostname='"
			+ defaultMessageHostname + '\'' + ", defaultSeverity=" + defaultSeverity + ", messageFormat="
			+ messageFormat + ", sentCounter=" + sentCounter + ", sentBytesEnveloped=" + sentBytesEnveloped
			+ ", sendDurationInNanosCounter=" + sendDurationInNanosCounter + ", sendErrorCounter=" + sendErrorCounter
